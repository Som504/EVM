...
begin
    if(reset)
        counter <= 0; // whenever reset passes so coumter start from 0
    else if(valid_vote_casted) // if valid vote is casted then counter becomes 1
        counter <= counter + 1;
...
begin
    ...
    begin
        if(mode == 0 & counter>0) //mode0 --> voting mode
            ...
            leds  <= 8'h00;
        else if(mode == 1) //mode1 --> Result Mode
        begin
